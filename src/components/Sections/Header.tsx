...
const Header: FC = memo(() => {
  ...
  const navSections = useMemo(
    () => [SectionId.About, SectionId.Portfolio, SectionId.Articles, SectionId.Testimonials, SectionId.Contact],
    [],
  );
...
const DesktopNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    ...
    const inactiveClass = classNames(baseClass, 'text-black');
    return (
      <header className="fixed top-0 z-50 hidden w-full bg-neutral-950/50 p-4 backdrop-blur sm:block" id={headerID}>
        <div className="relative flex items-center justify-center">
          ...
          {/* Left-side site name */}
          <span className="absolute left-4 text-white font-bold">enioborges.com</span>
        </div>
      </header>
    );
  },
...
const MobileNav: FC<{navSections: SectionId[]; currentSection: SectionId | null}> = memo(
  ({navSections, currentSection}) => {
    ...
    const activeClass = classNames(baseClass, 'bg-neutral-900 text-black font-bold');
    const inactiveClass = classNames(baseClass, 'text-neutral-200 font-medium');
    return (
      <>
        {/* Mobile header text (left side) */}
        <div className="hidden sm:fixed sm:left-8 sm:top-2 sm:z-40 sm:text-white sm:font-bold">enioborges.com</div>
